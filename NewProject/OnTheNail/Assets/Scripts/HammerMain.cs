...
public class HammerMain : MonoBehaviour
{
    ...
    void Update()
    {
        if (introScene_)
        {
            if (Application.platform == RuntimePlatform.Android || testingMobile)
            {
                HammerInputMobile();
            }
            else
    ...
    private void HammerInputMobile()
    {
        //SWING INPUT
        int i = 0;

        if (Input.touchCount > 0)  //Touched the screen
        {
            //SPIN INPUT
            if (Input.GetTouch(i).position.x > Screen.width / 2 && swinging_ == true)
